...
def get_file_path(ticker, start_year, end_year):
    cik = cik_matching_ticker(ticker, headers=headers)
    cleaned_cik = cik.lstrip('0')

    # 연도별로 필요한 정보를 저장할 딕셔너리 생성
    paths = {}
    base_path = "datasets/EXTRACTED_FILINGS"

    index = end_year - start_year + 1
    ...
    for year in range(start_year, end_year + 1):
        ...
        paths[year] = f"{base_path}/{cleaned_cik}_10k_{year}_{ac}.json"
...
if __name__ == "__main__":
    ...
    file_paths = get_file_path(ticker, startYear, endYear)
    result = {}
